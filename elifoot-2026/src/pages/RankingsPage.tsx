...
import { Trophy, Star, Zap, TrendingUp, Award, Shield, BarChart2 } from 'lucide-react';
          ...
          <div className="space-y-4">
            {recentAwards.map((award) => (
              <div key={award.season} className="border border-white/10 rounded-lg p-3">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                  ...
                  {award.boladeOuro && (
                    <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-2 text-center">
                      <div className="text-[10px] uppercase text-yellow-400/70 mb-1">🏆 Bola de Ouro</div>
                      <Star className="w-4 h-4 text-yellow-400 mx-auto mb-1 fill-yellow-400" />
                      ...
                      {award.boladeOuro.totalVotes && (
                        <div className="text-[9px] text-white/30 mt-0.5">{award.boladeOuro.totalVotes} pts · {award.boladeOuro.firstPlaceVotes} 1ºs</div>
                      )}
                    </div>
                  )}
                  {award.revelacao && (
                    <div className="bg-white/5 rounded-lg p-2 text-center">
                      <div className="text-[10px] uppercase text-white/40 mb-1">🎖 Troféu Kopa</div>
                      ...
                      <div className="text-xs text-green-400 font-bold mt-1">{award.revelacao.age} anos · OVR {award.revelacao.overall}</div>
                    </div>
                  )}
                  {award.melhorGoleiro && (
                    <div className="bg-white/5 rounded-lg p-2 text-center">
                      <div className="text-[10px] uppercase text-white/40 mb-1">🧤 Troféu Yashin</div>
                      <Shield className="w-4 h-4 text-blue-400 mx-auto mb-1" />
                      <div className="text-xs font-semibold truncate">{award.melhorGoleiro.playerName}</div>
                      <div className="text-[10px] text-white/50">{award.melhorGoleiro.teamName}</div>
                      <div className="text-xs text-blue-400 font-bold mt-1">GK · OVR {award.melhorGoleiro.overall}</div>
                    </div>
                  )}
                  {award.melhorTecnico && (
                    <div className="bg-white/5 rounded-lg p-2 text-center">
                ...
                {/* Top 5 votação Bola de Ouro */}
                {award.topVoters && award.topVoters.length > 1 && (
                  <div className="mt-3 pt-3 border-t border-white/10">
                    <div className="text-[10px] uppercase text-white/30 mb-2 flex items-center gap-1">
                      <BarChart2 className="w-3 h-3" /> Votação Bola de Ouro
                    </div>
                    <div className="space-y-1">
                      {award.topVoters.map((v, i) => (
                        <div key={i} className="flex items-center gap-2 text-xs">
                          <span className={`w-4 font-bold ${i === 0 ? 'text-yellow-400' : 'text-white/30'}`}>{i + 1}</span>
                          <span className="flex-1 truncate text-white/80">{v.playerName}</span>
                          <span className="text-white/40 text-[10px]">{v.teamName}</span>
                          <span className={`font-bold tabular-nums ${i === 0 ? 'text-yellow-400' : 'text-white/50'}`}>{v.points}</span>
                        </div>
                      ))}
                    </div>
                  </div>
